...
import com.study.jpa.chap05_practice.dto.PostDetailResopnseDTO;
...
public class PostApiController {
    ...
    public ResponseEntity<?> detail(@PathVariable Long id){
        ...
        try {
            PostDetailResopnseDTO dto = postService.getDetail(id);
            return ResponseEntity.ok().body(dto);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> create(
        ...
        // 위에 존재하는 if문을 모두 건너 뜀 -> dto가 null도 아니고, 입력값 검증도 모두 통과함.->service에게 명령
        try {
            PostDetailResopnseDTO responseDTO = postService.insert(dto);
